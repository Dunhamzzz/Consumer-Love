...
<ul class="full-category-list">
    <?php foreach ($categories as $parentCategory): ?>
        <li><?php echo $this->Link->category($parentCategory['Category']); ?> <?php
    echo $this->Html->link(
            $this->Html->image('icons/pencil.png', array('class' => 'inline-icon')), array(
        'action' => 'admin_edit',
            ...
            <?php if (!empty($parentCategory['children'])): ?>
                <ul>
                    <?php foreach ($parentCategory['children'] as $childCategory): ?>
                        <li><?php echo $this->Link->category($childCategory['Category']); ?> <?php
            echo $this->Html->link(
                    $this->Html->image('icons/pencil.png', array('class' => 'inline-icon')), array(
                'action' => 'admin_edit',
