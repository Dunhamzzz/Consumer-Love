...
if ($target) {
	...
	echo $form->create(null, array('url' => $ajaxUrl, 'target' => $target, 'id' => 'comment-form'));
} else {
...
if ($target) {
	echo $js->submit(__d('comments', 'Save', true), array_merge(array('url' => $ajaxUrl), $commentWidget->globalParams['ajaxOptions']));
} else {
	echo $form->submit(__d('comments', 'Save', true));
}
echo $form->end();
